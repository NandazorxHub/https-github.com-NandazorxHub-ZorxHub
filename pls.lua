...
--// PLS DONATE - CLEAN UI GAME + BLOCK REAL PURCHASE
--// CoreGui Roblox AMAN, UI Game ILANG, Popup Asli Roblox ILANG
...
-- ============ BAGIAN 1: MATIKAN POPUP PEMBELIAN ASLI ROOBLOX ============
pcall(function()
    StarterGui:SetCoreGuiEnabled(Enum.CoreGuiType.PurchasePrompt, false)
    StarterGui:SetCoreGuiTypeEnabled(Enum.CoreGuiType.PurchasePrompt, false)
    print("✅ PurchasePrompt dimatikan")
end)

-- HOOK MARKETPLACE BIAR POPUP GAK MUNCUL
pcall(function()
    MarketplaceService.PromptProductPurchase = function() 
        print("🔒 Popup beli asli diblokir!")
        return 
    end
    MarketplaceService.PromptPurchaseCompleted = function() 
        return 
    end
    print("✅ Marketplace dihook")
end)

-- ============ BAGIAN 2: HANCURIN POPUP YANG TERLANJUR MUNCUL ============
local function destroyRealPurchasePopup()
    for _, obj in pairs(CoreGui:GetDescendants()) do
        local name = obj.Name:lower()
        if name:find("purchase") or name:find("product") or name:find("checkout") or
           name:find("confirmpurchase") or name:find("robuxpurchase") or
           obj:IsA("Dialog") then
...
-- ============ BAGIAN 3: HANCURIN UI GAME PLS DONATE ============
local targetNames = {
    "Announcement", "Invite", "Bronze", "Fan", "BuyItem", "RobuxPls",
    "BoothFrame", "ClaimBooth", "EarnFrame", "ArrowGuide"
}

local targetKeywords = {
    "announcement", "invite", "bronze", "fan", "buy", "robux", "booth",
    "claim", "earn", "arrow", "free", "your text here"
}

-- UI ROOBLOX ASLI YANG DILINDUNGI
local protectedNames = {
    "RobloxGui", "CoreGui", "Chat", "PlayerList", "TopBar", "Health",
    "Backpack", "Settings", "Notifications"
}

-- UI PUNYA LU YANG DILINDUNGI
local myScripts = {
    "TopCounter",
    "SimpleUI",
    "FakeRobloxBuy",
...
local function isProtected(obj)
    local name = obj.Name
    for _, protected in pairs(protectedNames) do
        if name == protected then return true end
    end
    for _, scriptName in pairs(myScripts) do
...
notif.Text = "🔒 REAL PURCHASE BLOCKED + UI CLEANED\nPop up asli Roblox TIDAK akan muncul"
...
print("✅ SEMUA BERES!")
print("📌 UI Game Pls Donate: BERSIH")
print("📌 Popup beli asli Roblox: BLOKIR TOTAL")
print("📌 Script lu (FakeRobloxBuy, dll): AMAN")