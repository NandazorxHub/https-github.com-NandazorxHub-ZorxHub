...
-- ANTI DOUBLE GUI (No 1)
if game.CoreGui:FindFirstChild("NzkRedBlack") then
game.CoreGui:FindFirstChild("NzkRedBlack"):Destroy()
end

if game.CoreGui:FindFirstChild("NZStats") then
game.CoreGui:FindFirstChild("NZStats"):Destroy()
end
...
local sT = Instance.new("UIStroke", f); sT.Color = Color3.fromRGB(192,192,192); sT.Thickness = 1  
...
task.spawn(function()
while halo.Parent do
TweenService:Create(haloStroke, TweenInfo.new(0.6), {
Transparency = 0.4
}):Play()
...
addBtn("🛡️ AUTO BLOCK NEW", staffScroll, function()
    loadstring(game:HttpGet("https://raw.githubusercontent.com/dinhthanhtuankiet1762009-sudo/Js/refs/heads/main/96c7a36b9ce2a9ea.lua"))()
    zorxNotif("Auto Block External Loaded")
end)
...
-- AIMLOCK UI
local aimContainer = Instance.new("Frame", mainGui) aimContainer.Size = UDim2.new(0, 140, 0, 45); aimContainer.Position = UDim2.new(1, -150, 0, 70); aimContainer.BackgroundTransparency = 1
...
RunService.RenderStepped:Connect(function()
if aimlockOn and not lockTarget then
return -- 🔥 cegah ambil target baru
end

if aimlockOn and lockTarget then  

    applyHighlight(lockTarget)  

    -- 🔥 CEK TARGET MASIH HIDUP  
    local char = lockTarget.Parent  
    ...
    local targetPos = lockTarget.Position + Vector3.new(0,1.5,0)  
...
logo.BackgroundTransparency = 1  
logo.Image = "rbxassetid://100384294052265"  
logo.ZIndex = 999  
