...
-- TextBox untuk mengubah Robux visual
local robuxInput = Instance.new("TextBox", frame)
robuxInput.Size = UDim2.new(0, 80, 0, 30)
robuxInput.Position = UDim2.new(1, -50, 0, 5) -- tepat di samping robuxText
robuxInput.BackgroundColor3 = Color3.fromRGB(40,40,40)
robuxInput.TextColor3 = Color3.new(1,1,1)
robuxInput.Text = tostring(playerRobux) -- default = saldo asli
robuxInput.TextScaled = true
robuxInput.Font = Enum.Font.SourceSansSemibold
robuxInput.ClearTextOnFocus = false
robuxInput.TextXAlignment = Enum.TextXAlignment.Center
robuxInput.PlaceholderText = "0"

-- Fungsi untuk mengambil jumlah Robux visual dari TextBox
local function getVisualRobux()
	local value = tonumber(robuxInput.Text)
	if value and value >= 0 then
		return value
	else
		return playerRobux -- fallback ke saldo asli
	end
end

	local function updateRobux()
    robuxText.Text = formatNumber(getVisualRobux()) -- pakai nilai dari TextBox
end
...
price.Font = Enum.Font.SourceSansSemibold
	...
	playerRobux -= itemPrice -- kurangi saldo asli
	updateRobux()
